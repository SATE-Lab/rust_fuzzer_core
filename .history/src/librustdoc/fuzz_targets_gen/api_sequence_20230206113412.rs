...
impl ApiSequence<'_> {
    ...
    fn extract_function(tcx: TyCtxt<'_>, function: LocalDefId, full_name: String) -> Api<'_> {
        // 获取hir::Node
        let hir = tcx.hir().find_by_def_id(function).unwrap();
        ...
        */
        // 参数
        let arguments = Self::extract_input();
        let ret_value = Self::extract_output();

        Api { full_name, local_def_id, params: _, is_unsafe: _ }
    }

    fn extract_input<'tcx>() -> Vec<Param<'tcx>> {
        _
    }
