...
impl<'tcx> FormatRenderer<'tcx> for Context<'tcx> {
    ...
    fn init(
        ...
        if target_dir_name == "target" {
            ...
            let mut api_graph = ApiGraph::new(&krate.name(tcx).to_string());
            let mut full_name_map = impl_util::FullNameMap::new();
            impl_util::extract_impls_from_cache(&cache, tcx, &mut full_name_map, &mut api_graph);
...
impl Context<'_> {
    fn analyse_clean_krate(
        ...
        //let mut all = AllTypes::new();
        {
            ...
            while let Some((mut cx, item)) = work.pop() {
                cx.another_analyse_item(tcx, item, &mut api_graph, |cx, item| {
        ...
        F: FnMut(&mut Context<'_>, clean::Item),
    {
        if item.is_mod() {
            ...
            let mod_name = self.current.iter_mut().map(|x| x.to_string()).collect::Vec<String>().join("::");
            api_graph.add_mod_visibility(&mod_name, item.visibility(tcx).unwrap());
