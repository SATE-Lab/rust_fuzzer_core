...
use crate::fuzz_targets_gen::dependency::{extract_dependencies, print_all_dependencies};
...
impl<'tcx> FormatRenderer<'tcx> for Context<'tcx> {
    ...
    fn init(
        ...
        let _ = tcx.sess.time("build_call_graph", || {
            let _all_dependencies = extract_dependencies(tcx);
            print_all_dependencies(tcx, _all_dependencies);
    ...
    fn after_krate(&mut self) -> Result<(), Error> {
        println!("Finish parsing, Begin to build graph");
        Ok(())
    }
