...
impl ToAfl {
    ...
    fn get_all_dependency_fuzz_types(&self) {
        for ty in self.tys {
    ...
    //进行去重操作
    fn collect_all_unique_fuzz_types(&self) {
        for ty in self.tys {
    ...
    fn generate_all_u8_to_other_type_functions(&self) -> String {
        ...
        for ty in self.all_tys {
            res += &match ty {
                ...
                FuzzType::Slice(_) if !have_slice => Self::generate_u8_to_other_type_function(&ty),
                other => Self::generate_u8_to_other_type_function(&ty),
            }
        }
        res
    }
