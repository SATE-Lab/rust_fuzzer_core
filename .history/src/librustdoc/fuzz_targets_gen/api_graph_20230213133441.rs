...
impl<'a> ApiGraph<'a> {
    pub(crate) fn new(_crate_name: &String, cache: &'a Cache) -> Self {
        ...
        ApiGraph {
            ...
            functions_with_unsupported_fuzzable_types: FxHashSet::new(),
            cache,
    ...
    pub(crate) fn _try_deep_bfs(&mut self, max_sequence_number: usize) {
        ...
        let mut already_covered_nodes = FxHashSet::new();
        let mut already_covered_edges = FxHashSet::new();
        //接下来开始从长度1一直到max_len遍历
        for len in 0..max_len {
    ...
    pub(crate) fn _try_to_cover_unvisited_nodes(&mut self) {
        ...
        let mut unvisited_nodes = FxHashSet::new();
        ...
        let mut covered_node_this_iteration = FxHashSet::new();
        //最多循环没访问到的节点的数量
