...
pub fn _same_type_hard_mode<'tcx>(output_type: &Ty<'tcx>, input_type: &Ty<'tcx>) -> CallType<'tcx> {
    ...
    match input_type.kind() {
        TyKind::Ref(_, type_, mutbl) => {
            //
            return _borrowed_ref_in_same_type(mutbl, type_, output_type);
        }
        TyKind::RawPtr(typeandmut) => {
            let type_ = typeandmut.ty;
            let mutbl = &typeandmut.mutbl;
            return _raw_pointer_in_same_type(mutbl, type_, output_type, full_name_map);
        }
