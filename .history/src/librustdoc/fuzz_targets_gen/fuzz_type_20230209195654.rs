...
use super::{call_type::CallType, impl_util::FullNameMap, prelude_type::PreludeType};
...
impl FuzzableCallType {
    pub fn generate_fuzzable_type_and_call_type(&self) -> (FuzzType, CallType) {
        //println!("fuzzable call type: {:?}", self);
        match self {
            FuzzableCallType::NoFuzzable => (FuzzType::NoFuzzable, CallType::_NotCompatible),
            FuzzableCallType::Primitive(primitive) => {
                (FuzzType::Primitive(primitive.clone()), CallType::_DirectCall)
            }
            FuzzableCallType::Tuple(types) => {
                ...
                for type_ in types {
                    ...
                    if let FuzzType::NoFuzzable = fuzzable_type {
                        return (FuzzType::NoFuzzable, CallType::_NotCompatible);
                    }
                    match call_type {
                ...
                return (FuzzType::Tuple(fuzzable_types), CallType::_DirectCall);
            }
            ...
            FuzzableCallType::BorrowedRef(type_) => {
                ...
                if let FuzzableCallType::Slice(slice_inner) = inner_type {
                    ...
                    return (FuzzType::RefSlice(Box::new(fuzzable_type)), CallType::_DirectCall);
                }
                //一般的引用
                let (fuzzable_type, inner_call_type) =
            ...
            FuzzableCallType::STR => {
                return (FuzzType::RefStr, CallType::_DirectCall);
            }
