...
impl<'tcx> FormatRenderer<'tcx> for Context<'tcx> {
    ...
    fn init(
        ...
        Ok((Context { current: Vec::new(),graph: CrateGraph { krate: krate.name(tcx), functions: Vec::new() }
             _tcx: tcx, _cache: Rc::new(cache) }, krate))
    }
    ...
    fn item(&mut self, item: clean::Item) -> Result<(), Error> {
        ...
        match *item.kind {
            clean_types::ItemKind::FunctionItem(_) => {
                ...
                let _function = Function::create(full_name.clone(), item);
