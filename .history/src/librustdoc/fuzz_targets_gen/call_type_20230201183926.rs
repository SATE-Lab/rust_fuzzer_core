...
pub enum CallType {
    _NotCompatible,
    _DirectCall,                                  //直接调用 x
    _BorrowedRef(Box<CallType>),                  //取不可变引用 &(x)
    _MutBorrowedRef(Box<CallType>),               //取可变引用 &mut (x)
    _ConstRawPointer(Box<CallType>, clean::Type), //转换为不可变裸指针 &(x) as *const
    _MutRawPointer(Box<CallType>, clean::Type),   //转换为可变裸指针 &(x) as *mut
    _AsConvert(String),                           //通过as进行转换 x as string
    _UnsafeDeref(Box<CallType>),                  //解引用裸指针 *(x)
    _Deref(Box<CallType>),                        //解引用引用 *(x)
    _UnwrapResult(Box<CallType>),                 //获得result变量的ok值 _unwrap_result(x)
    _ToResult(Box<CallType>),                     //产生一个result类型 Ok(x) never used
    _UnwrapOption(Box<CallType>),                 //获得option变量的值 _unwrap_option(x)
    _ToOption(Box<CallType>),                     //产生一个option类型 Some(x)
}
...
impl CallType {
    ...
    //是否包含unsafe的调用
    pub fn is_unsafe_call_type(&self) -> bool {
        match self {
