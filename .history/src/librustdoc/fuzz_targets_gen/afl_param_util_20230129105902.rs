//! 这个部分可以生成关于函数参数相关的语句和函数
//!     1. 通过 AflForType 生成 api 用到的参数的类型对应的转换函数
//!         用法：let all_helper_function_definition_string = AflForType::generate(tys);
//!
//!     2. 通过 AflForParam 生成参数
//!         用法：let param_statement = generate_param_initial_statement(...)
//!         生成内容：let _param3 = (_to_i32(data, 8), _to_slice(data, 111, 113))
//!
...
impl AflForType {
    /// 核心函数
    /// let all_helper_function_definition_string = AflForType::generate(tys);
    pub fn generate(tys: Vec<FuzzType>) -> String {
        let mut x = Self::new(tys);
...
impl AflForParams for FuzzType {
    /// 生成参数初始化语句，如：
    /// let _param1 = _to_i32(data, 12);
    /// let _param2 = _to_slice(data, 123, 126);
    /// let _param3 = (_to_i32(data, 8), _to_slice(data, 111, 113))
    fn generate_param_initial_statement(
        &self,
        param_index: usize,
        fixed_start_index: usize,
    ...
    /// FIXME: 平分是否合适？
    /// 其中fixed_start_index是当前fixed参数开始（前面的部分已经当作其他fixed参数了）
    /// 其中dynamic_start_index是dynamic参数开始位置
    fn generate_param_initial_rhs(
        &self,
        fixed_start_index: usize,
        dynamic_start_index: usize,
        ...
        match self {
            ...
            // _to_slice(data, start_index, end_index)
            // _to_str(data,start_index, end_index)
            Str | Slice(..) => {
            ...
            Tuple(inners) => {
                ...
                for (i, inner) in inners.iter().enumerate() {
                    ...
                    inner_fixed_start_index =
                        inner_fixed_start_index + inner.fixed_size_part_size();
                    inner_dynamic_param_index =
                        inner_dynamic_param_index + inner.dynamic_size_parts_number();
                }
                res.push_str(")");
                res
            }
