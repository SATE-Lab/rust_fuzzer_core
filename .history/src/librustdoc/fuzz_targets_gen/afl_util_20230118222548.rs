...
#[allow(dead_code)]
impl ToAfl {
    pub fn generate_all_u8_to_other_type_functions(tys: Vec<&FuzzType>) -> String {
        let map = FxHashSet::default();

        let res: String;
        for ty in tys {
            if map.contains(&*ty) {
                res += Self::generate_u8_to_other_type_function(ty);
                map.insert(tys.clone());
            }
        }
        res
    }

    fn generate_u8_to_other_type_function(fuzz_type: &FuzzType) -> &'static str {
        use FuzzType::*;
        match fuzz_type {
            U8 => _data_to_u8(),
            I8 => _data_to_i8(),
            ...
            Tuple(elements) => {
                ...
                for element in elements {
                    s += generate_u8_to_other_type_function(*element);
                }
                s.as_str()
            }
        }
