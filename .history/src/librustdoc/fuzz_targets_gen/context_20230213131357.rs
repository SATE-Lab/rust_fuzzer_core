...
impl<'tcx> FormatRenderer<'tcx> for Context<'tcx> {
    ...
    fn init(
        ...
        if target_dir_name == "target" {
            ...
            let _ret = cx.clone().analyse_clean_krate(tcx, &krate, &mut api_graph);
    ...
    //    F: FnMut(&mut Context<'_>, clean::Item),
    {
        if item.is_mod() {
            ...
            for item in m.items {
                ...
                //f(self, item);
                self.another_analyse_item(tcx, item, api_graph, work)
            }
            self.dst = prev;
