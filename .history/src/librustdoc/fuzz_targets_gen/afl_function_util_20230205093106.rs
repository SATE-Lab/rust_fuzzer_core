...
struct AflFunctionHelper<'tcx> {
    ...
    pub function_mut_tag: FxHashSet<usize>,   //表示哪些函数返回值需要mut标记
...
impl AflFunctionHelper<'_> {
    ...
    fn is_function_return_value_need_mut_tag(&self, i: usize) -> bool {
        self.function_mut_tag.contains(&i)
    }
    ...
    pub fn generate_test_function(&self, test_index: usize, indent_size: usize) -> String {
        ...
        if self._unsafe_tag {
            ...
            let unsafe_function_body = self.generate_function_body_string(
                indent_size + 4,
                param_prefix_str,
                local_prefix_str,
            );
            ...
            let function_body =
                self.generate_function_body_string(indent_size, param_prefix_str, local_prefix_str);
    ...
    pub fn generate_function_body_string(
        ...
        let full_name_map = &self.api_seq.full_name_map;
        ...
        for (i, api_call) in self.api_seq.sequence.iter().enumerate() {
            ...
            for (j, param) in api_call.params.iter().enumerate() {
                ...
                if call_type_array_len == 1 {
                    ...
                    let param_call_string =
                        call_type._to_call_string(&param_ident_string, &full_name_map);
                    param_strings.push(param_call_string);
                } else {
                    //如果有多层
                    let mut former_param_name = param_ident_string.clone();
                    let mut former_helper_line = String::new();
                    let mut helper_index = 1;
                    let mut call_type;
                    for k in 0..call_type_array_len {
                        call_type = &call_type_array[k];
                        ...
                        let helper_line = format!(
                            ...
                            body_indent,
                            helper_name,
                            call_type._to_call_string(&former_param_name)
                        );
                        if helper_index > 1 {
                            if !call_type._need_mut_tag() {
                    ...
                    let param_string = call_type._to_call_string(&former_param_name, full_name_map);
            ...
            if self.api_seq.have_no_success(i) {
                //如果没有后继
                //let _ = test_function123(123, _local1, _param2);
                res.push_str("let _ = ");
            } else {
                //如果有后继, _local的下标就是函数的序号
                //let mut _local2 = ...
                let mut_tag =
                    if self.is_function_return_value_need_mut_tag(i) { "mut " } else { "" };
                res.push_str(format!("let {}{}{} = ", mut_tag, local_param_prefix, i).as_str());
            }

            res.push_str(api_call.full_name.as_str());
