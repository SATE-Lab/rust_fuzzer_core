...
use rustc_std_workspace_std::marker::Tuple;
...
#[allow(dead_code)]
impl ToAfl {
    fn get_all_dependency_fuzz_type(ty: &FuzzType) -> Vec<FuzzType> {
        let types = Vec::new();
        ...
        match ty {
            ...
            Bool | U16 => {
                let mut u8_dependency = Self::get_all_dependency_fuzz_type(&U8);
                types.append(&mut u8_dependency);
            }

            I16 => {
                let mut i8_dependency = Self::get_all_dependency_fuzz_type(&I8);
                types.append(&mut i8_dependency);
            }
            U32 => {
                let mut u16_dependency = Self::get_all_dependency_fuzz_type(&U16);
                types.append(&mut u16_dependency);
            }
            I32 => {
                let mut i16_dependency = Self::get_all_dependency_fuzz_type(&I16);
                types.append(&mut i16_dependency);
            }
            U64 => {
                let mut u32_dependency = Self::get_all_dependency_fuzz_type(&U32);
                types.append(&mut u32_dependency);
            }
            I64 => {
                let mut i32_dependency = Self::get_all_dependency_fuzz_type(&I32);
                types.append(&mut i32_dependency);
            }
            U128 => {
                let mut u64_dependency = Self::get_all_dependency_fuzz_type(&U64);
                types.append(&mut u64_dependency);
            }
            I128 => {
                let mut i64_dependency = Self::get_all_dependency_fuzz_type(&I64);
            ...
            Isize => {
                let mut i64_dependency = Self::get_all_dependency_fuzz_type(&i32);
                types.append(&mut i32_dependency);
            }
            Char => {
            ...
            Tuple(eles) => {
                for ele in *eles {
                    res.append(&mut Self::get_all_dependency_fuzz_type(&ele));
                }
            }
        }
        res
    ...
    pub fn generate_all_u8_to_other_type_functions(&self) -> String {
