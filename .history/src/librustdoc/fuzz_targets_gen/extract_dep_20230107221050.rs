...
impl<'tcx> LocalDependencies<'tcx> {
    ...
    fn get<'mir>(function_body: &'mir mir::Body<'tcx>) -> Self {
...
pub fn extract_dependencies<'tcx>(tcx: TyCtxt<'tcx>) -> AllDependencies<'tcx> {
    ...
    for function in tcx.hir().body_owners() {
        ...
        //直接依赖
        let deps = LocalDependencies::direct_dependencies(&mir).propagate();
        ...
        for callsite in &callsites {
            ...
            //存入每个调用点的参数依赖关系！
            dependencies.push(CallerDependency { arg_sources, callee });
        }
...
pub fn print_all_dependencies<'tcx>(tcx: TyCtxt<'tcx>, all_dependencies: AllDependencies<'tcx>) {
    ...
    for (caller, function) in functions {
        ...
        for CallerDependency { arg_sources, callee } in dependencies {
            match callee {
