...
impl<'tcx> Dependencies<'tcx> {
    ...
    fn direct_dependencies<'mir>(function_body: &'mir mir::Body<'tcx>) -> Self {
        ...
        struct Assignments<'tcx, 'local> {
            locals_count: usize,
            constants: &'local Vec<&'tcx mir::Constant<'tcx>>,
...
fn extract_constant<'tcx>(function: &mir::Body<'tcx>) -> Vec<&'tcx mir::Constant<'tcx>> {
    ...
    impl<'tcx> Visitor<'tcx> for Constants<'tcx> {
        fn visit_constant(&mut self, constant: &'tcx mir::Constant<'tcx>, _: mir::Location) {
            ...
            let symbol = function
                ...
                .find(|debug| {
                    ...
                    match &debug.value {
                        ...
                        Composite { ty: _, fragments: _ } => false, //FIXME:
                    }
                })
