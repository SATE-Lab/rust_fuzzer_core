...
fn print_dependencies(dep : &IndexVec<mir::Local, BitVec>, s: &str){
...
fn extract_function_call<'tcx>(
    ...
    impl<'tcx, 'local> SearchFunctionCall<'tcx, 'local> {
        fn new(tcx: TyCtxt<'tcx>, caller: &'local mir::Body<'tcx>) -> Self {
            SearchFunctionCall { tcx, _caller: caller, callees: Vec::new() }
        }
    }
...
pub fn extract_dependencies<'tcx>(tcx: TyCtxt<'tcx>) -> AllDependencies<'tcx> {
    ...
    for function in tcx.hir().body_owners() {
        ...
        let get_origins = |from: mir::Local| /* -> impl Iterator<Item=Source> */ {
            //FIXME:
            println!("get_origins, from = {} ,", from.as_usize());
            deps.dependencies(from)
                .filter_map(|dep| {
                    ...
                    match dep {
                        ...
                        Argument(arg) => {
                            println!("go depends on arg local{}",arg.as_usize());
                             ...
                             callsites
                                 ...
                                 .map(|callsite| {
                                     ...
                                     match callsite.function {
                                         DirectCall(callee) => {
                                            //FIXME:
                                            println!("go local is {}, callee is {}",local.as_usize(), tcx.def_path_str(callee));
