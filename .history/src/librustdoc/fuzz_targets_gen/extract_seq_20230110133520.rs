        ...
        if let Some(caller_local) = caller.as_local() {
            ...
            if args.iter().all(|arg| arg.ty.is_primitive_ty()) {
                ...
                while !stack.is_empty() {
                    ...
                    for CalleeDependency {_ arg_sources, callee } in callee_dependency {
                        use super::extract_dep::Callee;
                        let (crate_name, def_id) = match callee {
                            Callee::DirectCall(def_id) => {
                                (tcx.crate_name(caller.krate).as_str(), def_id)
                            }
...
impl FunctionInfo<'_> {
    fn get_function_info<'tcx>(tcx: TyCtxt<'tcx>, function: LocalDefId) -> FunctionInfo<'_> {
