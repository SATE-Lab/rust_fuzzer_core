...
impl AflFunctionHelper {
    ...
    pub fn generate_afl_main(&self, test_index: usize) -> String {
        ...
        res.push_str(self.generate_afl_closure_body(1, test_index).as_str());
        res.push_str(indent.as_str());
        res.push_str("});\n");
        res.push_str("}\n");
        res
    ...
    /// 闭包 ||
    #[allow(dead_code)]
    fn generate_main_closure(&self, outer_tab: usize, test_index: usize) -> String {
        let mut res = String::new();
        ...
        for i in 0..self.fuzz_param_types.len() {
            if i != 0 {
