...
pub fn preluded_type<'tcx>(type_: &Ty<'tcx>) -> bool {
...
pub enum PreludeType<'tcx> {
    NotPrelude(Ty<'tcx>),
    PreludeOption(Ty<'tcx>),
    PreludeResult { ok_type: Ty<'tcx>, err_type: Ty<'tcx> },
}

impl PreludeType {
    pub fn from_type<'tcx>(type_: &Ty<'tcx>) -> Self {
        match type_ {
            TyKind::ResolvedPath { path, .. } => {
                if preluded_type(type_) {
                    let def_id = type_.def_id().unwrap();
    ...
    pub fn _get_final_type<'tcx>(&self) -> Ty<'tcx> {
        //获得最终的类型
        match self {
...
pub fn _prelude_type_need_special_dealing<'tcx>(type_: &Ty<'tcx>) -> bool {
