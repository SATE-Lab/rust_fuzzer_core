...
impl AflFunctionHelper<'_> {
    ...
    pub fn generate_function_body_string(
        ...
        for (i, api_call) in self.api_seq.sequence.iter().enumerate() {
            ...
            for (index, param) in api_call.params.iter().enumerate() {
                ...
                let param_name = match param.returned_by_full_name {
                    Some(_) => {
                        let mut s1 = local_param_prefix.to_string() + &(index.to_string());
                        s1 += &(index.to_string());
                        s1
                    }
                    None => {
