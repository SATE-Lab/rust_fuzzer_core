...
impl<'tcx> Dependencies<'tcx> {
    ...
    fn direct_dependencies<'mir>(function: &'mir mir::Body<'tcx>) -> Self {
        ...
        let constants: Vec<ty::Ty<'tcx>> = extract_constant(function).into_iter().collect();
        ...
        impl<'tcx, 'local> Assignments<'tcx, 'local> {
            fn new(
                ...
                Assignments { constants, dependencies, locals_count }
            }
        }
                    ...
                    match op {
                        ...
                        Constant(constant) => {
                            locals_count
                                + constants
                                    .iter()
                                    .position(|cst| cst.ty() == constant.literal.ty())
                                    .unwrap()
                        }
                    }
                };
        ...
        Dependencies { dependencies, constants, arg_count: function.arg_count }
    }
...
fn extract_constant<'tcx>(function: &mir::Body<'tcx>) -> HashSet<ty::Ty<'tcx>> {
    use mir::visit::Visitor;

    #[derive(Default)]
    struct Constants<'tcx> {
        constants: HashSet<ty::Ty<'tcx>>,
    }
    impl<'tcx> Visitor<'tcx> for Constants<'tcx> {
        fn visit_constant(&mut self, constant: &mir::Constant<'tcx>, _: mir::Location) {
            self.constants.insert(constant.literal.ty().clone());
        }
    }
...
fn extract_function_call<'tcx>(
    ...
    impl<'tcx, 'local> SearchFunctionCall<'tcx, 'local> {
        fn new(tcx: ty::TyCtxt<'tcx>, caller: &'local mir::Body<'tcx>) -> Self {
            SearchFunctionCall { tcx, caller, callees: Vec::new() }
        }
    }
            ...
            let symbol = function
                ...
                .find(|debug| {
                    ...
                    match debug.value {
                        ...
                        Composite { ty, fragments } => false, //FIXME:
                    }
                })
...
fn is_callable(ty: &ty::Ty<'_>) -> bool {
