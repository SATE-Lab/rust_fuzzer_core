...
impl<'tcx> FormatRenderer<'tcx> for Context<'tcx> {
    ...
    fn init(
        ...
        if target_dir_name == "target" {
            ...
            let mut api_seq = ApiSequence::new();
            let mut full_name_map = impl_util::FullNameMap::new();
            impl_util::extract_impls_from_cache(&cache, tcx, &mut full_name_map, &mut api_seq);
...
impl Context<'_> {
    fn analyse_clean_krate(
        ...
        let krate_name = krate.name(self._tcx).clone();
        ...
        //let mut all = AllTypes::new();
        {
            ...
            while let Some((mut cx, item)) = work.pop() {
                cx.another_analyse_item(item, &mut api_seq, |cx, item| {
            ...
            if item_type == ItemType::Function {
                ...
                match item.inner {
                    clean::FunctionItem(ref func) => {
                        ...
                        api_seq.add_api_function(api_fun);
                    }
                    _ => {}
                }
            }
