...
impl ExtractSequence {
    ...
    pub fn extract_sequence<'tcx>(
        ...
        //遍历每一个本地函数
        let _ = all_dependencies.functions.iter().map(|(caller_def_id, function)| {
            // 满足两个条件:
            // 1. 需要当前crate的API
                        ...
                        for CalleeDependency { callee, callsite, .. } in callee_dependency {
                            ...
                            let (crate_name, callsite_def_id) = match callee {
                                Callee::DirectCall(def_id) => (
                                    tcx.crate_name(caller_def_id.krate).as_str().to_string(),
                                    def_id,
                                ),
                            ...
                            if crate_name.starts_with("url") {
                                // FIXME: 如果是test crate的api，推入序列
                                println!("11111211111111111111");
                            ...
                            if let Some(info) = FunctionInfo::new_by_callee_def_id(
                                ...
                                if !visit_set
                                    .contains(&tcx.def_path_str(callsite_def_id).to_string())
                                {
                                    // 否则存入stack供下次遍历
                                    stack.push(info);
                                }
    ...
    pub fn print_sequence(&self, enable: bool) {
        ...
        println!("Start to print sequence");
