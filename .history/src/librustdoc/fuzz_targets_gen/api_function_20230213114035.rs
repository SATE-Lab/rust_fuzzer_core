...
pub struct ApiFunction {
    ...
    pub generics: clean::Generics,        // 泛型
    pub inputs: Vec<clean::Type>,         //输入的参数
    pub output: Option<clean::Type>,      //返回值
    pub _trait_full_path: Option<String>, //Trait的全限定路径,因为使用trait::fun来调用函数的时候，需要将trait的全路径引入
    pub _unsafe_tag: ApiUnsafety,         //是否unsafe
}
...
impl ApiFunction {
    ...
    pub fn _is_end_function(&self, cache: &Cache, full_name_map: &FullNameMap) -> bool {
    ...
    /// 是否所有参数都是primitive
    pub fn _is_start_function(&self, cache: &Cache, full_name_map: &FullNameMap) -> bool {
    ...
    /// 打印函数
    pub fn _pretty_print(&self, cache: &Cache, full_name_map: &FullNameMap) -> String {
    ...
    /// 是否包含了未支持的类型
    pub fn contains_unsupported_fuzzable_type(
        &self,
        cache: &Cache,
