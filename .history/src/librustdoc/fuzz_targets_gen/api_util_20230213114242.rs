...
pub fn _is_end_type(ty: &clean::Type, cache: &Cache, full_name_map: &FullNameMap) -> bool {
    match ty {
        clean::Type::Path { .. } => {
...
pub fn _type_name(type_: &clean::Type, cache: &Cache, full_name_map: &FullNameMap) -> String {
...
pub fn is_fuzzable_type(ty_: &clean::Type, cache: &Cache, full_name_map: &FullNameMap) -> bool {
...
pub fn _resolved_path_equal_without_lifetime(ltype: &clean::Type, rtype: &clean::Type) -> bool {
    if let clean::Type::Path { path: lpath, did: ldid, is_generic: lis_generic, .. } = ltype {
        if let clean::Type::Path { path: rpath, did: rdid, is_generic: ris_generic, .. } = rtype {
