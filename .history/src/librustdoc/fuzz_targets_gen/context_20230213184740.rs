...
impl<'tcx> FormatRenderer<'tcx> for Context<'tcx> {
    ...
    fn init(
        ...
        if target_dir_name == "target" {
            ...
            let _ret = cx.clone().add_bare_functions_into_api_graph(tcx, &krate, &mut api_graph);
...
impl Context<'_> {
    ...
    fn another_analyse_item(
        mut self,
        tcx: TyCtxt<'_>,
        item: clean::Item,
        api_graph: &mut ApiGraph<'_>,
        //work: &'a mut Vec<(Context<'a>, clean::Item)>,
        //mut _f: F,
    ) -> Result<(), Error>
//where
    //    F: FnMut(&mut Context<'_>, clean::Item),
    {
        if item.is_mod() {
            ...
            for item in m.items {
                ...
                //f(self, item);
                let cx = self.clone();
                cx.another_analyse_item(tcx, item, api_graph)?
            }
            self.dst = prev;
