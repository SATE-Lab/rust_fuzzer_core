...
pub fn preluded_type(type_: &clean::Type, cache: &Cache, full_name_map: &FullNameMap) -> bool {
...
impl PreludeType {
    pub fn from_type(type_: &clean::Type, cache: &Cache, full_name_map: &FullNameMap) -> Self {
        match type_ {
            clean::Type::Path { path } => {
                if preluded_type(type_, cache, full_name_map) {
                    let def_id = type_.def_id(cache).unwrap();
    ...
    pub(crate) fn _to_type_name(&self, cache: &Cache, full_name_map: &FullNameMap) -> String {
        match self {
            PreludeType::NotPrelude(type_) => api_util::_type_name(type_, cache, full_name_map),
            PreludeType::PreludeOption(type_) => {
                let inner_type_name = api_util::_type_name(type_, cache, full_name_map);
                format!("Option<{}>", inner_type_name)
            }
            PreludeType::PreludeResult { ok_type, err_type } => {
                let ok_type_name = api_util::_type_name(ok_type, cache, full_name_map);
                let err_type_name = api_util::_type_name(err_type, cache, full_name_map);
