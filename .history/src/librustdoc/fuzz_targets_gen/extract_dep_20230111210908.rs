...
use super::extract_seq::CalleeInfo;
...
pub struct CalleeDependency<'tcx> {
    ...
    pub callee_info: Option<CalleeInfo<'tcx>>,
}
...
pub fn extract_all_dependencies<'tcx>(tcx: TyCtxt<'tcx>) -> AllDependencies<'tcx> {
    ...
    for function in tcx.hir().body_owners() {
        ...
        for callsite in &callsites {
            ...
            let mut callee_info;
            use LocalCallType::*;
            let (callee, callee_info) = match callsite.function {
                DirectCall(callee) => {
                    let return_ty = match callsite.return_variable{
                        Some(local)=>{},
                        None=>{}
                    }
                    let callee_info = CalleeInfo::new();
                    (Callee::DirectCall(callee), callee_info)
                }
                LocalFunctionPtr(ptr) => {
                    (Callee::LocalFunctionPtr(get_origins(ptr).collect()), None)
                }
            };
