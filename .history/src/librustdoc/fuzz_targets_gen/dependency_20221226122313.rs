            ...
            fn visit_assign(
                ...
                let get_id = |op: &mir::Operand<'tcx>| -> usize {
                    ...
                    match op {
                        ...
                        Constant(constant) => {
                            locals_count
                                + constants.iter().position(|cst| cst == constant.literal).unwrap()
                        }
                    }
                };
...
fn extract_constant<'tcx>(function: &mir::Body<'tcx>) -> HashSet<ty::ConstKind<'tcx>> {
    use mir::visit::Visitor;

    #[derive(Default)]
    struct Constants<'tcx> {
        constants: HashSet<&'tcx mir::Constant<'tcx>>,
    }
