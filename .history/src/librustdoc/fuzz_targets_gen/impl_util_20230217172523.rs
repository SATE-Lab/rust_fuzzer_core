...
use rustc_middle::ty::query::query_keys::visibility;
...
pub(crate) struct CrateImplCollection {
    ...
    //FIXME: 带泛型参数的impl块，但self是否该视为泛型？
...
fn full_path(paths: &Vec<Symbol>) -> String {
...
pub(crate) fn _analyse_impl(
    ...
    for item in inner_items {
        ...
        match &*item.kind {
            //FIXME: 这段代码暂时没用了，impl块里面的是method item，而不是function item,暂时留着，看里面是否会出现function item
            ...
            clean::MethodItem(_method, _) => {
                let decl = _method.decl.clone();
                let clean::FnDecl { inputs, output, .. } = decl;
                let _generics = _method.generics.clone();
            ...
            _ => {
                //println!("no covered item {:?}", &item.kind);
            }
        }
    }
}
...
fn replace_self_type(self_type: &clean::Type, impl_type: &clean::Type) -> clean::Type {
    ...
    match self_type {
        ...
        clean::Type::Path { path } => {
            ...
            let mut new_segments = ThinVec::new();
            for path_segment in segments {
