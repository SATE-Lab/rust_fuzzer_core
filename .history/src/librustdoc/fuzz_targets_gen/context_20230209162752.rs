...
impl<'tcx> FormatRenderer<'tcx> for Context<'tcx> {
    ...
    fn init(
        ...
        if target_dir_name == "target" {
            ...
            let _ = tcx.sess.time("parse tested crate and generate test file", || {
                let sequences: Vec<Vec<String>> = Vec::new();
            });
    ...
    fn after_krate(&mut self) -> Result<(), Error> {
        ...
        impl_util::extract_impls_from_cache(self.cache(), &mut full_name_map, &mut api_sequence);
        Ok(())
    }
