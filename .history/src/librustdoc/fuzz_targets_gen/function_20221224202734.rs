...
pub struct Argument {
    pub(crate) type_: clean_type::Type,
    ...
    fn from(item: clean_types::Item) -> Self {
        match *item.kind {
            clean_types::ItemKind::FunctionItem(func) => {
                ...
                let clean_types::FnDecl (inputs, output, ) = func
                let inputs = function_from_trait_argument_helper(&(*func.decl.inputs));
...
//把clean::types::Argument解析成本mod的Argument，方便使用
