...
impl ApiSequence {
    ...
    pub fn _dead_code(&self, _api_graph: &ApiGraph<'_>) -> Vec<bool> {
    ...
    pub fn _contains_dead_code_except_last_one(&self, _api_graph: &ApiGraph<'_>) -> bool {
        let sequence_len = self.len();
        if sequence_len <= 1 {
            return false;
        }
    ...
    pub fn _to_replay_crash_file(&self, _api_graph: &ApiGraph<'_>, test_index: usize) -> String {
        let mut res = self._to_afl_except_main(_api_graph, test_index, _api_graph.cache);
    ...
    pub fn _to_afl_test_file(&self, _api_graph: &ApiGraph<'_>, test_index: usize) -> String {
        let mut res = self._to_afl_except_main(_api_graph, test_index);
        res.push_str(self._afl_main_function(test_index).as_str());
        res
    }

    pub fn _to_libfuzzer_test_file(&self, _api_graph: &ApiGraph<'_>, test_index: usize) -> String {
        let mut res = self._to_afl_except_main(_api_graph, test_index);
        res = res.replace(
            "#[macro_use]\nextern crate afl;\n",
    ...
    pub fn _to_afl_except_main(&self, _api_graph: &ApiGraph<'_>, test_index: usize) -> String {
        let mut res = String::new();
        ...
        res.push_str(self._to_well_written_function(_api_graph, test_index, 0, cache).as_str());
        res.push('\n');
        res
    }
        ...
        for i in 0..api_calls_num {
            ...
            for j in 0..param_size {
                ...
                if call_type_array_len == 1 {
                    ...
                    let param_string = call_type._to_call_string(&param_name, cache, full_name_map);
                    ...
                    for k in 0..call_type_array_len - 1 {
                        ...
                        let helper_line = format!(
                            ...
                            body_indent,
                            helper_name,
                            call_type._to_call_string(&former_param_name, cache, full_name_map)
                        );
                        if helper_index > 1 {
                    ...
                    res.push_str(former_helper_line.as_str());
                    let param_string =
                        last_call_type._to_call_string(&former_param_name, cache, full_name_map);
                    param_strings.push(param_string);
                }
            }
